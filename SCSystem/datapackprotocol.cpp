...
char *get16String(unsigned long num)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        temp[i] = (num << 4 * i) >> 28;  //28是因为每次把需要的4字节都移到最高位了
        temp[i] = temp[i] >= 0 ? temp[i] : temp[i] + 16;  //temp[i]可能是负值（最高位为1），将其转为0-15之间
...
// 10进制转16进制
//15=0x15
unsigned char Int10ToHex16(unsigned int num)
{
...
// 16进制数转10进制数
//0x15=15
int Hex16ToInt10(unsigned char ch)
{
    int result = 0;
...
/***************************************************************************/
unsigned char CalHexCS(unsigned char *pdata, unsigned int len)
{
    unsigned char cs = 0;
    unsigned int i = 0;
    for (i = 0; i<len; i++)
...
// 计算校验和，累加和
//p为开始字节的位置,len=所有字节数
unsigned char CalCS(unsigned char *p, unsigned int len)
{
    unsigned char result = 0x00;
    unsigned int i = 0;
    for (i = 0; i<len; i++)
...
/***************************************************************************/
unsigned int CalU32CS(unsigned char *p, unsigned int len)
{
...
//返    回：ASCII字符
/////////////////////////////////////////////////////////////////////
unsigned char CharToASC(unsigned char bHex)
{
    if ((bHex <= 9)) bHex += 0x30;
...
//返    回：16进制
/////////////////////////////////////////////////////////////////////
unsigned char ASCToChar(unsigned char asc)
{
...
int CharP2ASC(unsigned char *pascii, unsigned char *pchar, int len)
{
    int i;
...
//len=ascii长度
int ASCP2Char(unsigned char *pasc, unsigned char *pchar, int len)
{
    int i, thislen;
...
*         0: Error
*/
int Str2Int(unsigned char *inputstr, int *intnum)
{
    int value = 0;
...
/***************************************************************************/
//高低位相反
unsigned char CharBitAllChange(unsigned char ch)
{
    int i;
...
/***************************************************************************/
//高低位相反
void BufBitAllChange(unsigned char *ch, unsigned int datalen)
{
    unsigned int i;
    for (i = 0; i<datalen; i++)
    {
...
/***************************************************************************/
//检查IP地址类型
unsigned char CheckAddrType(unsigned char *pdata, unsigned int len)
{
...
extern U08 protolDataParse(UCHAR *dataBuf,int dataLen, dataFrameContent *p)
{
    ...
    U32 cs = 0;
    //qDebug()<<"the SOH_VALUE is " << SOH_VALUE << "..." << UPLINK_PROTOL_GET_SOH(dataBuf);
    if(SOH_VALUE!=UPLINK_PROTOL_GET_SOH(dataBuf))//数据包头不符合
    {
        qDebug()<<"the SOH error!!!";
        return 0;
    }
